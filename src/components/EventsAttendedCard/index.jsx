...
const EventsAttendedCard = ({ image, title, subtitle, location, date }) => {
  ...
  return (
    <div className="rounded-lg p-4 border border-[#EEEEEE] bg-white hover:shadow-xl transition-shadow duration-200">
      ...
      <div className="px-1 pt-4">
        <p className="text-xs font-normal capitalize">{subtitle}</p>
        <h3 className="text-base font-medium mb-3 capitalize">{title}</h3>
        <div className="space-y-2">
          <div className="flex items-center text-gray-600 text-xs gap-4">
            <span>{location}</span>
            {date && (
